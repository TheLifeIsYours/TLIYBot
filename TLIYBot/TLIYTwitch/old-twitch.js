...
const getTLIYClientID = (msg) => {
    for(const [index, guild] of TheLifeBot.guilds.entries()){
        console.log("CommanCenter ID: ", TheLifeBot.guilds[index].categorys.TheLifeBot.CommandCenter.id);
        return TheLifeBot.guilds[index].categories.TheLifeBot.CommandCenter.id;
    }
}

//Discord stream notifications channel
const notifie_discord = (args) => {
    str = args.str != undefined ? args.str : null;
    file = args.file != undefined ? args.file : null;
    msg = args.msg != undefined ? args.msg : null;
    
    TLIYBotClientID = "446366878852448267";//getTLIYClientID(msg);
    return client.channels.get(TLIYBotClientID).send(str, {file: file});
};
...
//Matches and functions for discord bot
const keyWords = [
    {regex: /-(-)?(commands)?(help)?/gm, str: '--commands/help', funct: (msg) => {listCommands()}},
    // {regex: /watch/gm, str: 'watch', funct: (msg) => {addStreamListener(msg)}},
    // {regex: /listen/gm, str: 'listen', funct: (msg) => {addStreamListener(msg)}},
    {regex: /stream add/gm, str: 'stream add', funct: (msg) => {addStreamListener(msg)}},
    {regex: /add/gm, str: 'stream add', funct: (msg) => {addStreamListener(msg)}},
    {regex: /stream remove/gm, str: 'stream remove', funct: (msg) => {removeStreamListener(msg)}},
    {regex: /pick/gm, str: 'pick', funct: (msg) => {pickGame(msg)}},
    {regex: /ping/gm, str: 'ping', funct: (msg) => {msg.reply('pong')}}
];
...
client.on('message', msg => {
    ...
    if (Imgur.check(msg)) {
        let channel = client.channels.get("333676194937438219");
        Imgur.sendImgurImages(channel, msg);
    }
});
